...
public class ExpressionWithChildrenTestVisitor<T extends IVisitable & HasChildren> extends BaseTestSuiteExpressionVisitor<T> {
    @Override
    public void visit(T item) {
        for (Expressions expression : item.getItems()) {
            addParameters(expressionToParameters(expression));
        }
    }
}
