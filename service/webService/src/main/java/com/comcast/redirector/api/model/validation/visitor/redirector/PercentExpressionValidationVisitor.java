...
@ModelValidationVisitor(forClass = Percent.class)
public class PercentExpressionValidationVisitor extends BaseExpressionValidationVisitor<Percent>{
    @Override
    public void visit(Percent item) {
        if (item.getValue() <= 0 || item.getValue() > 100) {
            getValidationState().pushError(ValidationState.ErrorType.PercentRangeError);
        }
    }
}
