...
public class StartDispatcherTask implements IProcessTask {

    @Override
    public Boolean handle(Context context) throws Exception {
        String serviceName = context.getServiceName();

        DataSourceUtil.createRulesAndUrlRulesNode(serviceName);

        ZookeeperModelReloadDispatcher zookeeperModelReloadDispatcher = new ZookeeperModelReloadDispatcher(serviceName);
