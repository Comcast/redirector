...
public class StaticStackRegistrationTask implements IProcessTask {
    private static final Logger log = LoggerFactory.getLogger(StaticStackRegistrationTask.class);

    @Override
    public Boolean handle(Context context) {
        String serviceName = context.getServiceName();
        if (E2EConfigLoader.getDefaultInstance().getExcludedAppsFromStackAutoDiscovery().contains(serviceName)) {
            StackBackup stackBackup = context.getStackBackup();
            if(stackBackup == null) {
                log.error("Failed register stacks for application '{}'", serviceName);
                return false;
            }
