...
public class RunTestCasesTask implements IProcessTask {
    private final FilesPathHelper filesPathHelper;
    private final ITestCasesRunner testCasesRunner;

    public RunTestCasesTask(ITestCasesRunner testCasesRunner, FilesPathHelper filesPathHelper) {
    ...
    public Boolean handle(Context context) throws Exception {
        return testCasesRunner.performTestCases(context.getRedirectorTestCaseList(), context.getServiceName(), filesPathHelper);
    }
}
